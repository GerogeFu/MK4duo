  ...
  while (!response) {
    ...
    if (!response) {          // no "ok" was received in reserved time frame, user will fix the issue on mmu unit
      if (!mmu_print_saved) { // first occurence, we are saving current position, park print head in certain position and disable nozzle heater
        ...
        resume_hotend_temp = heaters[0].target_temperature;
        ...
        if (turn_off_nozzle) heaters[0].setTarget(0);
      ...
      if (turn_off_nozzle && resume_hotend_temp) {
        heaters[0].setTarget(resume_hotend_temp);
        LCD_MESSAGEPGM(MSG_HEATING);
        sound.playTone(200, 40);

        while (!heaters[0].wait_for_heating()) printer.safe_delay(1000);
      }
